...
def merge_lfq_files(lfq_files, key='peptide'):
    ...
    for file in lfq_files:
        ...
        print(f"Processing file: {file} with suffix: {file_suffix}, filename: {filename}")
        
        # Rename columns to include the suffix (except the key column, columns already containing the suffix, and columns matching the filename)
        df = df.rename(columns={
            col: f"{col}_{file_suffix}" 
            for col in df.columns 
            if col != key and file_suffix not in col and col != filename
        })
...
if __name__ == "__main__":
    ...
    if lfq_files:
        merged_data = merge_lfq_files(lfq_files)
        print("Merged Data:")
