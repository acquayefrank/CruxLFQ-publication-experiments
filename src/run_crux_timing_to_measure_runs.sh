...
mapfile -t spectra_files < <(find "$spectrum_files_folder" -type f -name "*.mzML" -printf '%p\n' | sort -u)
...
for i in {1..3}; do
    ...
    for num_files in 1 2 4 8 16 20; do
        ...
        crux_cmd="${crux} lfq ${assign_confidence_file} ${selected_files[@]} --overwrite T --spectrum-parser mstoolkit"
        /usr/bin/time -v $crux_cmd > "${LOG_FILE}" 2>&1
    done
done
